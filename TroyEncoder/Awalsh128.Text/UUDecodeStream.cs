...
namespace Awalsh128.Text
{
        ...
        : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            {
                FileName = SeekPreamble(this.stream, out this.unixLineEnding);
        ...
        private static string SeekPreamble(Stream encodedStream, out bool unixLineEnding)
        {
            ...
            {
                if ((c == ' ' || char.IsDigit((char)c)) && !file.Any()) continue;
                file.Add(c);
            }
            ...
            var fileName = new string(file.Select(x => (char)x).ToArray());
