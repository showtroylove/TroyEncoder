...
namespace Awalsh128.Text
{
    ...
    public static class UUEncoder
    {
        ...
        public static byte[] DecodeLine(Stream buffer)
        {
            ...
            var actualLineLength = buffer.Read(encodedLine, 0, encodedLine.Length);
            if (actualLineLength != encodedLine.Length)
            {
                throw new IOException(
                    $"Expected read length to be the encoded line length of {encodedLine.Length} but got {actualLineLength}.");
            }
            ...
            {
                var a = DecoderMap[encodedLine[encodedIndex]];
                var b = DecoderMap[encodedLine[encodedIndex + 1]];
                var c = DecoderMap[encodedLine[encodedIndex + 2]];
                var d = DecoderMap[encodedLine[encodedIndex + 3]];
        ...
        public static byte[] EncodeLine(byte[] buffer)
        {
            ...
            {
                var a = buffer[decodedIndex];
                var b = buffer[decodedIndex + 1];
                var c = buffer[decodedIndex + 2];
